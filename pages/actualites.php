...
<div class="table">
	<div id="ajouteCalendrier">
		<a href="./index.php?page=ajouteEvenement.php" class="btajout">Ajouter un évênement</a>
		<a href="./index.php?page=suppEvenement.php" class="btsupp">Supprimer un évênement</a>
	</div>
	</br></br>
	<div id="calendrier" class="table-cell">
	</div>
	<script>
	...
	<div id="actu" class="table-cell">
		<div id="ajouterActu">
			<a href="./index.php?page=ajouteActualite.php" class="btajout">Ajouter une actualité</a>
			<?php echo('<a href="./index.php?page=suppActualite.php&pseudo='.$_SESSION['pseudo'].'" class="btsupp">Supprimer vos actualités</a>'); ?>
		</div>
		</br>
		<div id="actualites"class="table-cell">
			<?php
				...
				while ($row = $requete->fetch() and $sup <5)
				{
					...
					while($row2 = $requete2->fetch())
					{
						if($i%2 == 1)
						{
							echo '<img class="left" align="left" src="data:'.$row2['typeImageActu'].';base64,'.base64_encode($row2['contenuImageActu']).'" alt="'.$row2['nomImage'].'" title="'.$row2['nomImageActu'].'" >';				
							$cote = "right";
						}else
						{
